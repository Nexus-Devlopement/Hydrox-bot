...
const { createBar, format, databasing, escapeRegex, isrequestchannel, getRandomInt, delay} = require("../musichandlers/functions"); //Loading all needed functions
const requestcmd = require("../musichandlers/requestcmds");
const {
  MessageEmbed
} = require(`discord.js`);
...
module.exports.run = async (client, message) => {
    ...
    if (cmd.length === 0){
      ...
      if(matchedPrefix.includes(client.user.id)) {
        ...
        .setTitle(`Hi, I Am **${client.user.username}** Need Help?`)
        .setThumbnail(client.user.displayAvatarURL({ dynamic: true}))
        .setDescription(`<:Spotify:933246700003409920> Enjoy Listening To Music By Using The \`-help\` Command.`)
        .addField('Invite Me',`
          You Can Add Me To Your Server By 
          [Clicking Here](https://discord.com/api/oauth2/authorize?client_id=849523433657466890&permissions=8&scope=bot)`)
        .addField('Support Server',`
          If You Have Questions, Suggestions, Or Found A Bug, Please Join The [Support Server](https://discord.gg/muji)!
        `)
      .setColor("RANDOM")
      .setTimestamp()
        );
    ...
    if (command){
        ...
        if (timestamps.has(message.author.id)) { //if the user is on cooldown
          ...
          if (now < expirationTime) { //if he is still on cooldonw
            ...
            not_allowed = true;
            return message.channel.send(`<:no:933239221836206131> Please wait ${timeLeft.toFixed(1)} more second(s) before reusing the \`${command.name}\` command.`); //send an information message
          }
        }
      ...
      try {
        ...
        if(command.memberpermissions) {
          if (!message.member.hasPermission(command.memberpermissions)) {
            try{ message.react("878695788040056852").catch(e => console.log("couldn't react this is a catch to prevent a crash".grey)); }catch{}
            not_allowed = true;
            message.channel.send(new Discord.MessageEmbed()
              .setColor(ee.wrongcolor)
              .setTitle("<:no:933239221836206131> You are not allowed to run this command!")
        ...
        if(command.parameters) {
          if(command.parameters.type == "music"){
            ...
            if(!message.guild.me.permissionsIn(channel.id).toArray().includes("CONNECT")){
              return message.channel.send(new MessageEmbed()
                .setColor(ee.wrongcolor)
                .setTitle(`<:no:933239221836206131> I am not allowed to connect to this Channel.`)
              );
            }
            if(!message.guild.me.permissionsIn(channel.id).toArray().includes("SPEAK")){
              return message.channel.send(new MessageEmbed()
                .setColor(ee.wrongcolor)
                .setTitle(`<:no:933239221836206131> I am not allowed to speak in this Channel to this Channel.`)
              );
            }
            if(channel.userlimit && channel.userlimit != 0 && channel.full)
              return message.channel.send(new MessageEmbed()
                .setColor(ee.wrongcolor)
                .setTitle(`<:no:933239221836206131> Your Voice Channel is full, I can't join!`)
              );
            ...
            if(!channel.joinable)
              return message.channel.send(new MessageEmbed()
                .setColor(ee.wrongcolor)
                .setTitle(`<:no:933239221836206131> I am not able to join your Channel!`)
              );
            ...
            if(command.parameters.activeplayer){
              if (!player){
                ...
                return message.channel.send(new MessageEmbed()
                  .setColor(ee.wrongcolor)
                  .setTitle(`<:no:933239221836206131> There is nothing playing`)
                );
              }
              if (!mechannel){
                if(player) try{ 
                ...
                return message.channel.send(new MessageEmbed()
                  .setColor(ee.wrongcolor)
                  .setTitle(`<:no:933239221836206131> I am not connected to a Channel`)
                );
              }
            }
            //if no previoussong
            if(command.parameters.previoussong){
              if (!player.queue.previous || player.queue.previous === null){
                ...
                return message.channel.send(new MessageEmbed()
                  .setColor(ee.wrongcolor)
                  .setTitle(`<:no:933239221836206131> There is no previous song yet!`)
                );
              }
            }
            ...
            if (player && channel.id !== player.voiceChannel && !command.parameters.notsamechannel)
              return message.channel.send(new MessageEmbed()
                .setColor(ee.wrongcolor)
                .setTitle(`<:no:933239221836206131> You need to be in my voice channel to use this command!`)
        ...
        return message.channel.send(new Discord.MessageEmbed()
          .setColor(ee.wrongcolor)
          .setTitle("<:no:933239221836206131> Something went wrong while, running the: `" + command.name + "` command")
