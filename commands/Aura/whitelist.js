...
module.exports = {
  ...
  description: 'See a list of words on the words-list.',
    run: async (client, message) => {
        ...
        let wordlist = new Discord.MessageEmbed().setAuthor(`${message.author.username}`, `${message.author.avatarURL({dynamic:true})}`).setColor(colors.uptime).setTitle(`${message.guild.name} AnitWords List <:hydroxwarn:935778078188261406>`).setThumbnail(guild)
