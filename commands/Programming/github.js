    ...
    const ps = db.get(`guild_${message.guild.id}_prefix`) || ","
			...
			if (!repo)
				return message.channel.send({embed: new MessageEmbed()
					...
					.setTitle(`<:no:933239221836206131> You didn't provide a Repository`)
			...
			if (!username || !repository) 
				return message.channel.send({embed: new MessageEmbed()
					.setColor(es.wrongcolor)
					.setFooter(es.footertext, es.footericon)
					.setTitle(`<:no:933239221836206131> Repository must be in the form \`username/repository\``)
			...
			if (!body) 
				return message.channel.send({embed: new MessageEmbed()
					.setColor(es.wrongcolor)
					.setFooter(es.footertext, es.footericon)
					.setTitle(`<:no:933239221836206131> Could not fetch that repo`)
