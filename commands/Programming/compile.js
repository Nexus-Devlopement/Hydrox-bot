      ...
      const ps = db.get(`guild_${message.guild.id}_prefix`) || ","
			...
			if (!lang || !code) 
				return message.channel.send({embed: new MessageEmbed()
					...
					.setTitle(`<:no:933239221836206131>  You didn't provide a Valid Code`)
			...
			if (!possiblecommands[lang]) 
				return message.channel.send({embed: new MessageEmbed()
					.setColor("RED")
					.setFooter("thanks for using me")
					.setTitle(`<:no:933239221836206131>  You provide an Invalid Language`)
			...
			return message.channel.send(new MessageEmbed()
			  .setColor("RED").setFooter("thanks for using me")
