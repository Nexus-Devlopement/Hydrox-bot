...
const emoji = require(`../../botconfig/emojis.json`);
  ...
  run: async (client, message, args) => {
    ...
    return message.channel.send(new MessageEmbed()
      ...
      .setTitle(`<:no:933239221836206131> Please Add a TIME!`)
    ...
    let time = 0;
      try {
        ...
        return message.channel.send(new MessageEmbed()
          .setColor("RANDOM")
          .setFooter("Thanks For Using Me!")
          .setTitle(`<:no:933239221836206131> please add a valid TIME!`)
          .setDescription(` Usage: \`${prefix}remind <Time+Format(e.g: 10m)> ++ TEXT\`\n\nExample: \`${prefix}remind 10m 32s ++ Remind me!!\``)
        );
      }
    ...
    if (returntime > 2073600000) return message.channel.send(new MessageEmbed()
      .setColor("RANDOM")
      .setFooter("Thanks For Using Me")
      .setTitle(`<:no:933239221836206131> The time limit is 24 Days!`)
      .setDescription(` Usage: \`${prefix}remind <Time+Format(e.g: 10m)> ++ TEXT\`\n\nExample: \`${prefix}remind 10m 32s ++ Remind me!!\``)
    );
