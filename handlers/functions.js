  ...
  try {
    ...
    if (!response || response.loadType === 'LOAD_FAILED' || response.loadType !== 'PLAYLIST_LOADED') {
      let embed = new MessageEmbed()
        .setTitle("<:no:933239221836206131> Found nothing related for the latest Song!")
...
function create_join_to_create_Channel(client, user, type) {
  ...
  if (!user.guild.me.hasPermission("MANAGE_CHANNELS")) {
    allowed = false;
    try {
      user.member.user.send("${user.member.user} | <:no:933239221836206131> Error | Please give me the permission, `MANGE CHANNELS` --> I need to be able to create Channels ...")
    } catch {
      try {
        ...
        channel.send(`${user.member.user} | <:no:933239221836206131> Error | Please give me the permission, \`MANGE CHANNELS\` --> I need to be able to create Channels ...`).catch(e => console.log("THIS IS TO PREVENT A CRASH"))
      } catch {}
    }
  }
  if (allowed) {
