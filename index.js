...
client.on('guildCreate', guild => {
  const owner = guild.owner;
   ...
   if(!owner) return console.log("Cannot fetch guild owner!");
    ...
    .setTitle(`<a:Join_vc:934506868469039145> Joined A New Server | ${guild.name}`)
    .setDescription(`<:joines:933723116822220821> **${guild.name}** | (\`${guild.id}\`)`)
    .setThumbnail(guild.iconURL({ dynamic: true }) || null)
    .addField("> Server Owner", `> <a:r_dot:935066570630262854> ${guild.owner.user.username} | ${guild.owner}`)
    .addField("> Membercount", `> <a:r_dot:935066570630262854> ${guild.memberCount}`)
    .addField("> Server Bot Is In", `> <a:r_dot:935066570630262854> ${client.guilds.cache.size}`)
    .addField("<:leave:933722870734024724> Get Bot Out Of There -", `\`\`\`leaveserver ${guild.id}\`\`\``)
    .setFooter("Thanks For Using Me!")
    .setColor("RANDOM")
    .setTimestamp()
  try {
...
client.on('guildDelete', guild =>{
  ...
  const leaveembed = new MessageEmbed()
    .setTitle(`<:leave:933722870734024724> Left a Guild | ${guild.name}`)
    .setDescription(`<:joines:933723116822220821> **${guild.name}** | (\`${guild.id}\`)`)
    .setThumbnail(guild.iconURL({ dynamic: true }) || null)
    .addField("> Server Owner", `> <a:r_dot:935066570630262854> ${guild.owner.user.username} | ${guild.owner}`)
    .addField("> MemberCount", `> <a:r_dot:935066570630262854> ${guild.memberCount}`)
...
client.on('clickButton', async (b) => {
  const user = b.clicker.user.username;
      ...
      .setDescription(`<a:yes:933239140718358558> Verify The Step To Close The Ticket!`)
      .setColor("GREEN")
      .setFooter("Thanks For Using Me")
      .setTimestamp()
